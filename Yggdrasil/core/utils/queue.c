...
void queue_push(queue_t* q, queue_t_elem* elem) {
	...
	if(elem->type == LK_REQUEST) {
		LKRequest* tmp = (LKRequest*)(target->data + (target->writer * target->element_size));
		if(tmp->length > 0) {
		...
		LKTimer* tmp = (LKTimer*)(target->data + (target->writer * target->element_size));
		if(tmp->length > 0) {
		...
		LKEvent* tmp = (LKEvent*)(target->data + (target->writer * target->element_size));
		if(tmp->length > 0) {
