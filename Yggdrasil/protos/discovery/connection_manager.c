...
#define RETRY_TIME 30*60*1000*1000 //30 mins in us
...
static unsigned long off = 30;

static void setTimer(LKTimer* timer, unsigned long time_us, unsigned long repeat_us){
	LKTimer_init(timer, myProtoID, myProtoID);
	LKTimer_set(timer, time_us, repeat_us);
...
static int checkTryAgain(lk_ft_neigh* n){
	//TODO RETRY TIME converted to seconds
	return (n->lastCall + RETRY_TIME/(1000*1000)) - time(NULL);
}
...
static int checkTime(lk_ft_neigh* n){
	//TODO off converted to seconds
	return (n->lastCall + (off/(1000*1000))) - time(NULL);
}
...
static void checkQuarantine(){
	while(quarentine != NULL && checkTime(quarentine) <  0){
		...
		if(threshold > 0 && tmp->faults > threshold){
			blockNeigh(tmp);
		}else{
#ifdef DEBUG
			char msg[2000];
	...
	if(quarentine != NULL){
		...
		while(tmp->next != NULL){
			if(checkTime(tmp->next) < 0){
				...
				if(threshold > 0 && tmp2->faults > threshold)
					blockNeigh(tmp2);
				else{
#ifdef DEBUG
					char msg[2000];
		...
		if(uuid_compare(tmp->uuid, elem->uuid) == 0){
			...
			while(tmp->next != NULL){
				if(uuid_compare(tmp->next->uuid, elem->uuid) == 0){
					...
					free(elem); //Was duplicated in the process can be freed;
					if(threshold > 0 && tmp->next->faults > threshold){
...
static void discov_ft_lb_send_ev_up(lk_ft_neigh* neigh) {
	LKEvent ev;
	ev.notification_id = DISCOV_FT_LB_EV_NEIGH_UP;
	ev.proto_origin = myProtoID;
	ev.length = WLAN_ADDR_LEN + sizeof(uuid_t);
	ev.payload = malloc(ev.length);

	memcpy(ev.payload, neigh->uuid, sizeof(uuid_t));
	memcpy(ev.payload + sizeof(uuid_t), neigh->addr.data, WLAN_ADDR_LEN);
...
void* connection_manager_init(void* args) {
	proto_args* pargs = args;
	myProtoID = pargs->protoID;

	connection_attr* attr = (connection_attr*) pargs->protoAttr;
	queue_t* dispatcher_queue = attr->intercept;
	unsigned long discovery_period = attr->discovery_period;
	unsigned short msg_lost_per_fault = attr->msg_lost_per_fault;
	threshold = attr->black_list_links;

	queue_t* inBox = pargs->inBox;

	queue_t_elem elem;

	unsigned long timeout_time = discovery_period * msg_lost_per_fault;

	off = timeout_time * threshold * 2;

	LKTimer timer;

	setTimer(&timer, discovery_period, discovery_period);

	LKTimer gman;

	if(off > 0)
		setTimer(&gman, off, off);


	LKTimer unblocker;
	if(threshold > 0){
		setTimer(&unblocker, RETRY_TIME, RETRY_TIME);
	}
		...
		if(elem.type == LK_MESSAGE) {
			if(elem.data.msg.LKProto != myProtoID) { //This message is not for me therefore is going to the network
				...
				if(read == sizeof(uuid_t)) {
					if(database == NULL) {
						...
						lk_log("CONNECTION MANAGER", "NEW NODE", uuid_str);

						discov_ft_lb_send_ev_up(temp);
					} else {
			...
			if(uuid_compare(elem.data.timer.id, timer.id) == 0){
				...
				if(timeout_time > 0){
					...
					//TODO: timeout_time converted to seconds
					while(database != NULL && database->lastCall + (timeout_time/(1000*1000)) < now) {
						...
						neighborDownNotification.notification_id = DISCOV_FT_LB_EV_NEIGH_DOWN;
						...
						lk_log("CONNECTION MANAGER", "SUSPECT NODE", uuid_str);
					...
					if(database != NULL) {
						...
						while(db->next != NULL) {
							...
							if(db->next->lastCall + (timeout_time/(1000*1000)) < now) { //Timeout
			...
			lk_log("DISCOVERY_FAULTDETECTOR", "WARNING", s);
#endif
			queue_push(dispatcher_queue, &elem);
		}
