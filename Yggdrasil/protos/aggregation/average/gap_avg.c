...
static void updateentry(uuid_t n, WLANAddr addr, avg_val* w, int l, uuid_t p){
			...
			if(it_neigh != self) {
				if(parent != NULL) {
					if(it_neigh->level < parent->level){
						parent = it_neigh;
						char p_id[37];
						uuid_unparse(parent->neigh_id, p_id);
						//printf("parent: %s\n", p_id);
					}
				}else {
					parent = it_neigh;
...
void * gap_avg_init(void* args){
	...
	while(1){
		...
		if(elem.type == LK_MESSAGE){
			...
			updateentry(other.neigh, elem.data.msg.srcAddr, &other.val, other.level, other.parent);
			...
			if(elem.data.request.request == REQUEST){
				...
				if(elem.data.request.request_type == AGG_INIT){
					...
					double result;
					if(parent != NULL)
						result = parent->val.sum / parent->val.count;
					else
						result = 0;
