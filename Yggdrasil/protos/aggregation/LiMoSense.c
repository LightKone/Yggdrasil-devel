...
static neighs* addneigh(uuid_t n, WLANAddr* addr){
	...
	//	newnei->val.val = 0;
	//	newnei->val.weight = 0;
	newnei->cleared.val = 0;
	newnei->cleared.weight = 0;
	newnei->sent.val = 0;
	newnei->sent.weight = 0;
	newnei->recved.val = 0;
	newnei->recved.weight = 0;
	newnei->totalDiff.val = 0;
	newnei->totalDiff.weight = 0;
	newnei->inSN = 0;
	newnei->outSN = 0;
...
static void rmneigh(uuid_t n){
	...
	if(torm != NULL){
		if(torm->totalDiff.weight < 0){
			subtract_est(&my_est, &torm->totalDiff, &my_est);
		}else{
			sum_est(&unrecved, &torm->totalDiff, &unrecved);
		}
		neis--;
		free(torm);
	}
...
static void sendMsg(){
	...
	if(my_est.weight >= 2*q && tosend->totalDiff.weight > (-2)*bound && tosend->sent.weight < 2*bound){
		est tmp_est;
		tmp_est.val = my_est.val;
		tmp_est.weight = (my_est.weight / 2);
		sum_est(&tosend->sent, &tmp_est, &tosend->sent);

		subtract_est(&tosend->totalDiff, &tmp_est, &tosend->totalDiff);
	...
	LKMessage_addPayload(&msg, (void*)&tosend->outSN, sizeof(short)); //outSN_i(j)
	short toinSN = (tosend->inSN - 1)%2;
	LKMessage_addPayload(&msg, (void*)&toinSN, sizeof(short)); //inSN_i(j) - 1 mod 2
	LKMessage_addPayload(&msg, (void*)&tosend->cleared.val, sizeof(double)); //cleared_i(j)
	LKMessage_addPayload(&msg, (void*)&tosend->cleared.weight, sizeof(double));
...
static void processMsg(LKMessage* msg){
	...
	payload += sizeof(double);
	memcpy(&msgSN, payload, sizeof(short));
	payload += sizeof(short);
	memcpy(&clearSN, payload, sizeof(short));
	payload += sizeof(short);
	memcpy(&cleared_val.val, payload, sizeof(double));
	payload += sizeof(double);
	memcpy(&cleared_val.weight, payload, sizeof(double));
	...
	if(clearSN == nei->outSN){
		nei->outSN = (nei->outSN + 1) %2;
		subtract_est(&nei->sent,&cleared_val, &nei->sent);
	}
	...
	if(msgSN == nei->inSN){
		...
		sum_est(&nei->totalDiff, &diff, &nei->totalDiff);
		nei->recved.val = in_est.val;
		nei->recved.weight = in_est.weight;

		if(nei->recved.weight > bound){
			nei->inSN = (nei->inSN + 1) % 2;
			nei->cleared.val = nei->recved.val;
			nei->cleared.weight = nei->recved.weight;
			nei->recved.val = 0;
			nei->recved.weight = 0;
		}
	}
