...
tree_msgs_history* create_log_entry(tree_command* msg_header) {
	tree_msgs_history* entry = malloc(sizeof(tree_msgs_history));
...
void destroy_log_entry(tree_msgs_history* entry) {
...
void register_msg_reception(tree_msgs_history* entry, tree_command* msg_header) {
...
void add_announce_for_msg(tree_msgs_history* entry, peer_connection* sender) {
	if(entry->cmd == UNDEFINED) {
		if(entry->announces_size == 0) {
			entry->announces = malloc(sizeof(peer_connection*));
			entry->announces_timestamp = malloc(sizeof(time_t));
...
pthread_mutex_t* lock_peer(peer_connection* peer) {
	pthread_mutex_lock(&(peer->peer_lock));
	return &peer->peer_lock;
}

pthread_mutex_t* lock_peers() {
...
is_up_operation* create_is_up_operation_instance(tree_command* msg_header, peer_connection* source, short protocol_source) {
	...
	while(p!=NULL) {
		...
		if(p != source && p->state == CONNECTED) {
			if(p->tree == ACTIVE) {
				lk_log("CONTROL_COMMAND_TREE", "IS_UP REQUEST: Requesting value to peer", p->ip_addr);
...
int check_is_up_terminated(is_up_operation* is_up_instance) {
	int i;
...
unsigned int compute_is_up_return_value(is_up_operation* is_up_instance) {
	int v = 1, i = 0;
...
char* compute_get_neighbors_payload_to_return(is_up_operation* is_up_instance) {
...
char* compute_is_up_payload_to_return(is_up_operation* is_up_instance) {
	int i = 0;
...
void destroy_is_up_operation(is_up_operation* is_up_instance) {
...
void is_up_instance_register_peer_reply(is_up_operation* instance, peer_connection* p, unsigned int value, char* payload) {
	int i;
	...
	for(i = 0; i < instance->number_active_peers; i++) {
		if(instance->active_peers[i]->state == CONNECTED && instance->active_peers[i]->tree == ACTIVE && instance->value_peers[i] == 0) {
			lk_log("CONTROL_COMMAND_TREE", "IS_UP REPLY REGISTERED - Missing:", instance->active_peers[i]->ip_addr);
			missing += 1;
		}
	}
	...
	lk_log("CONTROL_COMMAND_TREE", "IS_UP REPLY REGISTERED", val);
#endif
...
CONTROL_COMMAND_TREE_REQUESTS get_operation_reply_code(CONTROL_COMMAND_TREE_REQUESTS op) {
	switch(op) {
	case IS_UP:
...
void reply_to_is_up_operation(is_up_operation* instance) {
	...
	if(instance->source != NULL) { //This request was received from a peer
		...
		if(instance->source->state == CONNECTED) {
#ifdef DEBUG
			lk_log("CONTROL_COMMAND_TREE", "IS_UP OPERATION TERMINATED -- UP-STREAM", instance->source->ip_addr);
#endif
			...
			//#ifdef DEBUG
			lk_log("CONTROL_COMMAND_TREE", "IS_UP OPERATION TERMINATED -- REPLY SENT TO UP-STREAM", instance->source->ip_addr);
			//#endif
		}
		unlock(lock);
	} else { //This request was received from a client, so we will return the reply directly to the client.
#ifdef DEBUG
		lk_log("CONTROL_COMMAND_TREE", "IS_UP OPERATION TERMINATED -- EVENT", "");
#endif
		LKRequest request;
		...
		//#ifdef DEBUG
		lk_log("CONTROL_COMMAND_TREE", "IS_UP OPERATION TERMINATED -- SENT EVENT", request.payload);
		//#endif
...
void reply_to_get_neighbors_operation(is_up_operation* instance) {
	unsigned int ret_value = compute_is_up_return_value(instance);
	...
	if(instance->source != NULL) { //This request was received from a peer
		...
		if(instance->source->state == CONNECTED) {
#ifdef DEBUG
			lk_log("CONTROL_COMMAND_TREE", "GET_NEIGHBORS OPERATION TERMINATED -- UP-STREAM", instance->source->ip_addr);
#endif
			tree_command* ret_msg = init_command_header(get_operation_reply_code(instance->cmd));
			ret_msg->command_size = sizeof(uuid_t) + sizeof(unsigned int) + sizeof(int) + payload_size;
			ret_msg->command = malloc(ret_msg->command_size);
			...
			destroy_command_header(ret_msg);
#ifdef DEBUG
			lk_log("CONTROL_COMMAND_TREE", "GET_NEIGHBORS OPERATION TERMINATED -- REPLY SENT TO UP-STREAM", instance->source->ip_addr);
#endif
		}
		unlock(lock);
	} else { //This request was received from a client, so we will return the reply directly to the client.
#ifdef DEBUG
		lk_log("CONTROL_COMMAND_TREE", "GET_NEIGHBORS OPERATION TERMINATED -- EVENT", "");
#endif
		LKRequest request;
		request.request = REPLY;
		request.request_type = instance->cmd;
...
void append_to_is_up_running_operations(is_up_operation* is_up_instance) {
...
is_up_operation* remove_is_up_operation(is_up_operation* is_up_instance) {
	is_up_operation** tmp = &running_operations;
...
is_up_operation* get_is_up_instance(uuid_t instance_id) {
	is_up_operation* tmp = running_operations;
	...
	while(*tmp != NULL) {
		...
		if(check_is_up_terminated(instance)) {
			if(instance->cmd == IS_UP || instance->cmd == RUN || instance->cmd == KILL || instance->cmd == REMOTE_CHANGE_VAL
				...
				lk_log("CONTROL_COMMAND_TREE", "PENDING OPERATION PROCESSING (by PRUNE)", "DO NOT KNOW HOW TO REPLY");
			}
			*tmp = instance->next;
...
tree_msgs_history* findEntry(uuid_t msg) {
...
void append_to_history(tree_msgs_history* entry) {
	tree_msgs_history** tmp = &history;
	while(*tmp != NULL) {
		tmp = &((*tmp)->next);
	}
...
void retransmit(peer_connection* p, tree_msgs_history* entry) {
...
void request_to_retransmit(peer_connection* p, tree_msgs_history* entry) {
	if(writefully(p->socket, entry->msg_id, sizeof(uuid_t)) <= 0)
		handle_peer_exception(p);
}

void check_missed_messages() {
	tree_msgs_history** tmp = &history;
	while(*tmp != NULL) {
...
static LKTimer history_garbage_collector;
static LKTimer missed_messages_check;

void* queue_monitor_thread(void* arg) {
	lk_log("CONTROL_COMMAND_TREE", "QUEUE_MONITOR_THREAD", "ACTIVE");
...
#ifdef DEBUG
		lk_log("CONTROL_COMMAND_TREE", "QUEUE_MONITOR_THREAD", "RECEIVED SOMETHING ON QUEUE");
#endif
...
void shutdown_protocol() {
	lk_log("CONTROL_COMMAND_TREE", "SHUTTING DOWN", "something unexpected happened");
	lk_logflush();
...
int isIPSmaller(char ip1[16], char ip2[16]) {
	if( strcmp(ip1, ip2) < 0)
		return 1;
	return 0;
}

peer_connection* findPeerConnection(char ip[16]) {
	pthread_mutex_t* previous = lock_peers();
	peer_connection* p = peers;
...
void* extablishConnectionAndDoHandshake(void* arg) {
#ifdef DEBUG
	lk_log("CONTROL_COMMAND_TREE", "extablishConnectionAndDoHandshake", ((peer_connection*) arg)->ip_addr);
#endif
	...
	if(sucess) {
		...
		unlock(lock);
		lk_log("CONTROL_COMMAND_TREE","New Peer Connection (init)", newPeer->ip_addr);
	} else {
		lock = lock_peer(newPeer);
		newPeer->state = ERROR;
		unlock(lock);
		handle_peer_exception(newPeer);
		lk_log("CONTROL_COMMAND_TREE", "Peer connection failed on connect()", newPeer->ip_addr);
	}

	return NULL;
}

void extablish_new_connection_to_peer(char ip[16]) {
#ifdef DEBUG
	lk_log("CONTROL_COMMAND_TREE", "Attempt to create connection", ip);
#endif
...
void handle_new_connection() {
#ifdef DEBUG
	lk_log("CONTROL_COMMAND_TREE", "ACCEPT THREAD", "Attempting to accept incoming TCP connection.");
#endif
	peer_connection* newPeer = malloc(sizeof(peer_connection));
	pthread_mutexattr_t attr;
	pthread_mutexattr_init(&attr);
...
#ifdef DEBUG
		lk_log("CONTROL_COMMAND_TREE", "Incoming socket addr", socket_addr);
#endif
...
#ifdef DEBUG
			lk_log("CONTROL_COMMAND_TREE", "Peer connection reception failed", "on write hostsize");
...
#ifdef DEBUG
			lk_log("CONTROL_COMMAND_TREE", "Peer connection reception failed", "on write hostname");
...
#ifdef DEBUG
			lk_log("CONTROL_COMMAND_TREE", "Peer connection reception failed", "on read ip addr");
...
#ifdef DEBUG
			lk_log("CONTROL_COMMAND_TREE", "Peer connection reception failed", "on read hostsize");
...
#ifdef DEBUG
			lk_log("CONTROL_COMMAND_TREE", "Peer connection reception failed", "on read hostname");
...
#ifdef DEBUG
		lk_log("CONTROL_COMMAND_TREE", "Peer connection reception failed", "on accept()");
	...
	if(error) {
...
#ifdef DEBUG
		lk_log("CONTROL_COMMAND_TREE", "Incoming peer addr", newPeer->ip_addr);
#endif
		...
		lk_log("CONTROL_COMMAND_TREE", "New Peer Connection (recv)", newPeer->ip_addr);
	}
}

void propagate(tree_command* cmd, peer_connection* exception) {
	lk_log("CONTROL_COMMAND_TREE", "PROPAGATE - START", my_ip_addr);
	...
	while(p != NULL) {
		...
		if(p->state == CONNECTED) {
			lk_log("CONTROL_COMMAND_TREE", "PROPAGATE - CONNECTED", p->ip_addr);
			if(p->tree == ACTIVE && p != exception) {
				lk_log("CONTROL_COMMAND_TREE", "PROPAGATE - SENT", p->ip_addr);
				write_command_header(p->socket, cmd);
				write_command_body(p->socket, cmd);
			} else if(p->tree == PASSIVE && p != exception) {
				lk_log("CONTROL_COMMAND_TREE", "PROPAGATE - ANNOUNCE", p->ip_addr);
	...
	unlock(previous);
	lk_log("CONTROL_COMMAND_TREE", "PROPAGATE", "END");
}

short process_msg(peer_connection* p, tree_command* msg, short protoID) {
	LKRequest request;
	is_up_operation* instance;

	switch(msg->command_code) {
	case SETUP:
		lk_log("CONTROL_COMMAND_TREE", "SETUP msg", p != NULL ? p->ip_addr : "LOCAL REQUEST");
		//Nothing extra to do
		return 1;
		break;
	case IS_UP:
		lk_log("CONTROL_COMMAND_TREE", "IS UP msg", p != NULL ? p->ip_addr : "LOCAL REQUEST");
#ifdef DEBUG
		char id[37];
		bzero(id, 37);
		uuid_unparse(msg->id, id);
		lk_log("CONTROL_COMMAND_TREE", "IS_UP target instance", id);
#endif
	...
	case KILL:
		...
		lk_log("CONTROL_COMMAND_TREE", "KILL msg", msg->command);
	...
	case RUN:
		//begin the test
		lk_log("CONTROL_COMMAND_TREE", "RUN msg", msg->command);
		instance = create_is_up_operation_instance(msg, p, protoID);
		if(check_is_up_terminated(instance)) {
			reply_to_is_up_operation(instance);
			destroy_is_up_operation(instance);
	...
	case REMOTE_CHANGE_LINK:
		...
		lk_log("CONTROL_COMMAND_TREE", "REMOTE_CHANGE_LINK msg", p != NULL ? p->ip_addr : "LOCAL REQUEST");
	...
	case REMOTE_CHANGE_VAL:
		...
		lk_log("CONTROL_COMMAND_TREE", "REMOTE_CHANGE_VAL msg", p != NULL ? p->ip_addr : "LOCAL REQUEST");
		...
		break;
	case REBOOT:
		lk_log("CONTROL_COMMAND_TREE", "REBOOT msg", p != NULL ? p->ip_addr : "LOCAL REQUEST");
		instance = create_is_up_operation_instance(msg, p, protoID);
		if(check_is_up_terminated(instance)) {
			reply_to_is_up_operation(instance);
			destroy_is_up_operation(instance);
		...
		break;
	case LOCAL_ENABLE_DISC:
		lk_log("CONTROL_COMMAND_TREE", "LOCAL_ENABLE_DISC msg", p != NULL ? p->ip_addr : "LOCAL REQUEST");
		request.proto_origin = protoID;
		...
		break;
	case LOCAL_DISABLE_DISC:
		lk_log("CONTROL_COMMAND_TREE", "LOCAL_DISABLE_DISC msg", p != NULL ? p->ip_addr : "LOCAL REQUEST");
		request.proto_origin = protoID;
		request.proto_dest = controlDiscoveryProtoID;
		request.length = 0;
		request.payload = NULL;
		...
		break;
	case SHUTDOWN:
		lk_log("CONTROL_COMMAND_TREE", "SHUTDOWN msg", p != NULL ? p->ip_addr : "LOCAL REQUEST");
		instance = create_is_up_operation_instance(msg, p, protoID);
		if(check_is_up_terminated(instance)) {
			reply_to_is_up_operation(instance);
			destroy_is_up_operation(instance);
		...
		break;
	case GET_NEIGHBORS:
		lk_log("CONTROL_COMMAND_TREE", "GET_NEIGHBORS msg", p != NULL ? p->ip_addr : "LOCAL REQUEST");
#ifdef DEBUG
		bzero(id, 37);
		uuid_unparse(msg->id, id);
		lk_log("CONTROL_COMMAND_TREE", "GET NEIGHBORS target instance", id);
#endif
		instance = create_is_up_operation_instance(msg, p, protoID);
		if(check_is_up_terminated(instance)) {
			reply_to_get_neighbors_operation(instance);
...
void printneighbortable() {
	...
	while(p != NULL) {
		...
		lk_log(p->hostname, p->ip_addr, p->state == CONNECTED ? ( p->tree == ACTIVE ? "CONNECTED - TREE ACTIVE" : "CONNECTED - TREE PASSIVE") : "NOT CONNECTED");
		fflush(stdout);
		p = p->next;
		unlock(previous);
		previous = lock;
...
void handle_queue_request() {
#ifdef DEBUG
	lk_log("CONTROL_COMMAND_TREE", "MAIN PROTOCOL", "HANDLING QUEUE REQUEST");
#endif
	queue_t_elem elem;
	...
	if(elem.type == LK_MESSAGE) {
...
#ifdef DEBUG
		lk_log("CONTROL_COMMAND_TREE", "MAIN PROTOCOL", "LK_TIMER");
#endif
		if(uuid_compare(elem.data.timer.id, history_garbage_collector.id) == 0) {
			...
			lk_log("CONTROL_COMMAND_TREE", "MAIN PROTOCOL", "Unknown timer");
		}
...
#ifdef DEBUG
		lk_log("CONTROL_COMMAND_TREE", "MAIN PROTOCOL", "LK_REQUEST");
#endif
		...
		if(req->request == REQUEST) {
			...
			case SETUP:
				lk_log("CONTROL_COMMAND_TREE", "LK_REQUEST", "SETUP");
				...
				break;
			case IS_UP:
				lk_log("CONTROL_COMMAND_TREE", "LK_REQUEST", "IS_UP");
				...
				bzero(id,37);
				uuid_unparse(instance->instance_id, id);
				lk_log("CONTROL_COMMAND_TREE", "Starting IS_UP operation", id);
#endif
				if(check_is_up_terminated(instance)) {
					reply_to_is_up_operation(instance);
					destroy_is_up_operation(instance);
			...
			case DO_COMMAND:
				//TODO EXEC LOCALLY
				lk_log("CONTROL_COMMAND_TREE", "LK_REQUEST", "DO_COMMND");
				cmd = init_command_header(DO_COMMAND);
				add_command_body(cmd, elem.data.request.payload, elem.data.request.length);
				append_to_history(create_log_entry(cmd));
				break;
			case KILL:
				lk_log("CONTROL_COMMAND_TREE", "LK_REQUEST", "KILL");
				cmd = init_command_header(KILL);
				add_command_body(cmd, elem.data.request.payload, elem.data.request.length);
				append_to_history(create_log_entry(cmd));
				process_msg(NULL, cmd, req->proto_origin);
#ifdef DEBUG
				lk_log("CONTROL_COMMAND_TREE", "PROCESSED KILL", cmd->command);
#endif
				destroy_command_header(cmd);
				break;
			case RUN:
				lk_log("CONTROL_COMMAND_TREE", "LK_REQUEST", "RUN");
				cmd = init_command_header(RUN);
				add_command_body(cmd, elem.data.request.payload, elem.data.request.length);
				append_to_history(create_log_entry(cmd));
				process_msg(NULL, cmd, req->proto_origin);
#ifdef DEBUG
				lk_log("CONTROL_COMMAND_TREE", "PROCESSED RUN", cmd->command);
#endif
				destroy_command_header(cmd);
				break;
			case REMOTE_CHANGE_LINK:
				lk_log("CONTROL_COMMAND_TREE", "LK_REQUEST", "REMOTE_CHANGE_LINK");
				cmd = init_command_header(REMOTE_CHANGE_LINK);
				add_command_body(cmd, elem.data.request.payload, elem.data.request.length);
				append_to_history(create_log_entry(cmd));
				process_msg(NULL, cmd, req->proto_origin);
#ifdef DEBUG
				lk_log("CONTROL_COMMAND_TREE", "PROCESSED CHANGE LINK", cmd->command);
#endif
				destroy_command_header(cmd);
				break;
			case REMOTE_CHANGE_VAL:
				lk_log("CONTROL_COMMAND_TREE", "LK_REQUEST", "REMOTE_CHANGE_VAL");
				...
				break;
			case REBOOT:
				lk_log("CONTROL_COMMAND_TREE", "LK_REQUEST", "REBOOT");
				cmd = init_command_header(REBOOT);
				append_to_history(create_log_entry(cmd));
				process_msg(NULL, cmd, req->proto_origin);
				break;
			case LOCAL_ENABLE_DISC:
				lk_log("CONTROL_COMMAND_TREE", "LK_REQUEST", "ENABLE_DISC");
				...
				break;
			case LOCAL_DISABLE_DISC:
				lk_log("CONTROL_COMMAND_TREE", "LK_REQUEST", "DISABLE_DISC");
			...
			case SHUTDOWN:
				...
				break;
			case GET_NEIGHBORS:
				lk_log("CONTROL_COMMAND_TREE", "LK_REQUEST", "GET_NEIGHBORS");
				...
				bzero(id,37);
				uuid_unparse(instance->instance_id, id);
				lk_log("CONTROL_COMMAND_TREE", "Starting GET_NEIGHBORS operation", id);
#endif
				if(check_is_up_terminated(instance)) {
					reply_to_get_neighbors_operation(instance);
					destroy_is_up_operation(instance);
			...
			case DEBUG_NEIGHBOR_TABLE:
				lk_log("CONTROL_COMMAND_TREE", "LK_REQUEST", "DEBUG_NEIGHBOR_TABLE");
				req->request = REPLY;
				req->proto_dest = req->proto_origin;
				req->proto_origin = protoID;
				if(req->length > 0 && req->payload != NULL)
...
#ifdef DEBUG
		lk_log("CONTROL_COMMAND_TREE", "MAIN PROTOCOL", "LK_EVENT");
#endif
		...
		lk_log("CONTROL_COMMAND_TREE", "MAIN PROTOCOL", s);
#endif
		if(event->proto_origin == controlDiscoveryProtoID && event->notification_id == NEW_NEIGHBOR_IP_NOTIFICATION) {
			if(isIPSmaller(my_ip_addr, (char*) event->payload) && isNotKnwonPeer((char*)event->payload)) {
				...
				lk_log("CONTROL_COMMAND_TREE", "MAIN PROTOCOL", "IGNORING PEER, NOT MY RESPONSABILITY OR ALREADY EXISTS");
			}
#endif
		}
...
int read_message_body(tree_command* msg, peer_connection* p) {
	switch(msg->command_code) {
	case DO_COMMAND:
	case KILL:
	case RUN:
...
void	 handle_peer_communication(peer_connection* p) {
	lk_log("CONTROL_COMMAND_TREE", "HANDLE PEER CONNECTION", p->ip_addr);
	...
	lk_log("CONTROL_COMMAND_TREE", "HANDLE PEER CONNECTION -> CMD CODE:", s);
#endif
	...
	switch(msg->command_code) {
	case NO_OP:
		lk_log("CONTROL_COMMAND_TREE", "Handle NO_OP (keep alive)", p->ip_addr);
		resend = 0;
		break;
	case LOCAL_PRUNE:
		lk_log("CONTROL_COMMAND_TREE", "Handle LOCAL_PRUNE message", p->ip_addr);
		p->tree = PASSIVE;
		check_running_operations_completion();
		break;
	case LOCAL_ATTACH:
		...
		if(target != NULL && target->cmd != UNDEFINED)
			...
			lk_log("CONTROL_COMMAND_TREE", "Handle LOCAL_ATTACH message", "Received request to retransmit something that is not in my historic");
		break;
	case LOCAL_ANNOUNCE:
		if(p->tree != PASSIVE) {
			lk_log("CONTROL_COMMAND_TREE", "Handle LOCAL_ANNOUNCE message", "Received announce from a neighbor that is not passive (correcting)");
			p->tree = PASSIVE;
		}
...
#ifdef DEBUG
		lk_log("CONTROL_COMMAND_TREE", "Handle LOCAL_IS_UP_REPLY", "Going to receive message payload.");
#endif
...
#ifdef DEBUG
		lk_log("CONTROL_COMMAND_TREE", "Handle LOCAL_IS_UP_REPLY", "Receive payload.");
#endif
		ptr = msg->command;
		...
		if(instance != NULL) {
			...
			if(check_is_up_terminated(instance)) {
				...
				if(instance != NULL)
					...
					lk_log("CONTROL_COMMAND_TREE", "Handle LOCAL_IS_UP_REPLY", "Could not remove is_up_instance from list.");
			}
		} else {
			lk_log("CONTROL_COMMAND_TREE", "Handle LOCAL_IS_UP_REPLY", "No valid instance");
		}
		break;
	case LOCAL_GET_NEIGHBORS_REPLY:
#ifdef DEBUG
		lk_log("CONTROL_COMMAND_TREE", "Handle LOCAL_GET_NEIGHBORS_REPLY", "Going to receive payload.");
#endif
		if(read_command_body(p->socket, msg) == -1) {
			//Error condition;
			handle_peer_exception(p);
...
#ifdef DEBUG
		lk_log("CONTROL_COMMAND_TREE", "Handle LOCAL_GET_NEIGHBORS_REPLY", "Receive payload.");
#endif
		...
		if(instance != NULL) {
			...
			if(check_is_up_terminated(instance)) {
				...
				if(instance != NULL)
					...
					lk_log("CONTROL_COMMAND_TREE", "Handle LOCAL_GET_NEIGHBORS_REPLY", "Could not remove is_up_instance from list.");
			}
		} else {
			lk_log("CONTROL_COMMAND_TREE", "Handle LOCAL_GET_NEIGHBORS_REPLY", "No valid instance");
		}
		break;
	default:
...
void* control_command_tree_init(void* arg) { //argument is the time for the announcement
	...
	controlDiscoveryProtoID = (unsigned short) ((long) pargs->protoAttr);
	...
	LKTimer_init(&history_garbage_collector, protoID, protoID); //repeat every hour (in us)
	LKTimer_set(&history_garbage_collector, ((unsigned long)1*60*60*1000*1000), ((unsigned long)1*60*60*1000*1000));
	setupTimer(&history_garbage_collector);

	LKTimer_init(&missed_messages_check, protoID, protoID);
	LKTimer_set(&missed_messages_check, (2*1000*1000), (2*1000*1000)); //every 2 seconds, check tree
