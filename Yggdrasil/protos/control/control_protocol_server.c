...
int executeSimpleOperation(int clientSocket, int OPERATION_CODE) {
	LKRequest req;
	req.length = 0;
	req.payload = NULL;
	...
	while(1) {
		...
		if( queue_try_timed_pop(inBox, &timeout, &elem) != 1 ) {
			...
			if(elem.type == LK_REQUEST && elem.data.request.request_type == OPERATION_CODE
		...
		if(elem.type == LK_REQUEST && elem.data.request.length > 0 && elem.data.request.payload != NULL)
...
int executeGenericOperation(int clientSocket, int OPERATION_CODE, char* payload, int payloadsize) {
	LKRequest req;
...
int executeOperationWithPayloadAndConfirmation (int clientSocket, int OPERATION_CODE) {
...
int executeOperationWithConfirmation(int clientSocket, int OPERATION_CODE) {
