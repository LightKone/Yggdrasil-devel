...
//#include "../protos/aggregation/flow_updating_single.h"
#include "../protos/aggregation/drg_single.h"
...
//	updateProtoAttr(PROTO_DISCOV_FT_LB, (void*) &attr);
	...
	drg_attr d_attr;
	d_attr.discovery_proto_id = PROTO_DISCOV_FT_LB;
	d_attr.reliable_proto_id = PROTO_RELIABLE;

	addProtocol(PROTO_DRG, &drg_single_init, (void*) &d_attr, 0);
...
//	registInterestEvents(PROTO_DISCOV_FT_LB, ev, 2, PROTO_FLOW_UPDATING);
	...
	while(1) {
		...
		if(elem.type == LK_MESSAGE) {
			...
			if(elem.data.request.proto_origin == PROTO_DRG && elem.data.request.request == REPLY && elem.data.request.request_type == AGG_VAL){
