...
//#include "../protos/aggregation/multi_root_aggregation.h"
#include "protos/aggregation/average/multi_root_avg.h"
...
int main(int argc, char* argv[]) {
	...
	addProtocol(PROTO_MULTI, &multi_root_avg_init, (void*) &discov, 0);
...
//	registInterestEvents(PROTO_TCP_AGG_SERVER,ev,1,myId);
	...
	registInterestEvents(PROTO_DISCOV_FT_LB, ev, 1, PROTO_MULTI);
	ev[0] = CHANGE_VAL;
	registInterestEvents(control_app, ev, 1, PROTO_MULTI);
	...
	lk_log("MULTI ROOT", "PARAMS", s);
	lk_log("MULTI ROOT", "SRDS18", "round my_val agg_val");
	...
	while(1) {
		...
		if(elem.type == LK_MESSAGE) {
			...
			if(elem.data.request.proto_origin == PROTO_MULTI && elem.data.request.request == REPLY && elem.data.request.request_type == AGG_VAL){
