...
#include "protos/aggregation/flow_updating.h"
...
int main(int argc, char* argv[]) {
	...
	addProtocol(PROTO_FLOW_UPDATING, &flow_updating_init, (void*) &discov, 0);
	...
	registInterestEvents(PROTO_DISCOV_FT_LB, ev, 2, PROTO_FLOW_UPDATING);

	ev[0] = CHANGE_VAL;
	registInterestEvents(control_app, ev, 1, PROTO_FLOW_UPDATING);
	...
	lk_log("FLOW", "PARAMS", s);
	lk_log("FLOW", "SRDS18", "round my_val agg_val");
	...
	while(1) {
		...
		if(elem.type == LK_MESSAGE) {
			...
			req.proto_dest = PROTO_FLOW_UPDATING;
			req.request = REQUEST;
			...
			if(elem.data.request.proto_origin == PROTO_FLOW_UPDATING && elem.data.request.request == REPLY && elem.data.request.request_type == AGG_VAL){
