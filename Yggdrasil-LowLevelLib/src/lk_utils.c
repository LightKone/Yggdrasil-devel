...
// Checks if a message within a buffer belongs to the Lightkone Protocol
int isLightkoneProtocolMessage(void* buffer, int bufferLen) {
	if(bufferLen < WLAN_HEADER_LEN+LK_HEADER_LEN)
		return 0;

	unsigned char* p = (buffer+WLAN_HEADER_LEN);
	unsigned char lk[LK_HEADER_LEN] = AF_LK_ARRAY;
	if(memcmp(p,lk,3)==0) {
		return 1;
	}
...
// Checks if a message within a buffer belongs to the Lightkone Protocol Timer
int isLightkoneProtocolTimer(void* buffer, int bufferLen) {
	if(bufferLen < WLAN_HEADER_LEN+LK_HEADER_LEN)
		return 0;

	unsigned char* p = (buffer+WLAN_HEADER_LEN);
	unsigned char lk[LK_HEADER_LEN] = AF_LKT_ARRAY;
